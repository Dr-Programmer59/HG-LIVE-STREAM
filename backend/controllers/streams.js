...
export const createStream = catchAsyncError(async (req, res) => {
	...
	const {title,thumnail,logo} = req.body;
  let thumnailpath,logopath;
  if(thumnail){
        const filterData = thumnail.substr(thumnail.indexOf(',')+1);
		const buffer = new Buffer(filterData,'base64');
        const filename = `thumbnail${new Date(Date.now()).getTime()}.png`
	    fs.writeFileSync(path.join(__dirname,`../public/upload/images/${filename}`),buffer,'binary');
		thumnailpath = `/upload/images/${filename}`;
    }
    if(logo){
      const filterData = logo.substr(logo.indexOf(',')+1);
      const buffer = new Buffer(filterData,'base64');
          const filename = `logo${new Date(Date.now()).getTime()}.png`
        fs.writeFileSync(path.join(__dirname,`../public/upload/images/${filename}`),buffer,'binary');
      logopath = `/upload/images/${filename}`;
    ...
    const stream = await StreamModel.create({title,owner: req.user._id,banner,thumnail: thumnailpath,logo:logopath});
